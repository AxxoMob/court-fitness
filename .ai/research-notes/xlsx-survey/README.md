...
**Status:** reference only. Results were captured and analysed in `.ai/domain-reference/ltat-fitness-findings.md` (§9) and `.ai/institutional-memory/HARD_LESSONS.md` (HL-9). The scripts are kept so any future agent who needs to re-inspect the workbooks can do so without rewriting this wiring.
