...
7. **Memory-to-repo promotion.** Before the close commit, scan your session-private agent memory (the owner cannot see this; it dies when the conversation ends) for any load-bearing fact — owner preferences, architectural decisions, newly-discovered domain constraints, unresolved questions — NOT already in `.ai/` repo docs. Promote each into the appropriate `.ai/core/*.md` file (or a new reference doc under `.ai/core/`). Repo docs are amnesia-proof; agent memory is not.
   ...
   **Binary design artifacts must be promoted the moment they arrive, not at session close.** Every screenshot, Figma export, design sketch, reference image, mockup, or video the owner shares in chat is saved to `.ai/research-notes/screenshots/` (images) or `.ai/research-notes/design/` (other binaries) within the same session it is received, with a sibling `.md` note recording: (1) what it shows, (2) who shared it, (3) when (date + session N), (4) which feature/screen it references, (5) any spoken words the owner said alongside it. Prose distillation is lossy — a screenshot encodes layout, proximity, proportion, and column count that no amount of text can preserve. Skipping this rule caused HL-13 (Session 5's Plan Builder was rebuilt from lossy prose because the original LTAT screenshots from Session 1 were never saved). This is artifact #7's binary half; treat it with the same gravity as the text-fact half above.
